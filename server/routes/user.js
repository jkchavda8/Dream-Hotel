...
router.patch("/:userId/:listingId", async (req, res) => {
  try {
    ...
    const favoriteListing = user.wishList.find((item) => item && item._id && item._id.toString() === listingId);

    if (favoriteListing) {
      user.wishList = user.wishList.filter((item) => item && item._id && item._id.toString() !== listingId);
      await user.save();
